...
#[allow(deprecated)]
use openssl::ssl::SslStreamBuilder;
use openssl::ssl::{SslMethod, SslSession};
...
fn test_early_exporter() {
        ...
        for _ in 0..ITERS {
            ...
            let message = if let Some(mut early) = server.early_data() {
                let secret = early
                    .exporter()
                    .unwrap()
                    .derive([0u8; 64], b"label", Some(b"context"))
                    .unwrap();
    ...
    for _iter in 0..ITERS {
        ...
        #[allow(deprecated)] // doesn't seem to be a way to call `write_early_data` early enough?
        let mut stream_builder = SslStreamBuilder::new(
            config
                .into_ssl("testserver.com")
                .unwrap(),
            stream,
        );
        let _ = stream_builder.write_early_data(b"early hello");

        let mut stream = stream_builder.connect().unwrap();
