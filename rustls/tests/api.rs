...
fn do_exporter_test(
    ...
    assert!(
        client_exporter
            .derive(&mut client_secret, b"label", Some(b"context"))
            .is_ok()
    );
    assert!(
        server_exporter
            .derive(&mut server_secret, b"label", Some(b"context"))
            .is_ok()
    );
    ...
    assert_eq!(
        client_exporter
            .derive(&mut empty, b"label", Some(b"context"))
            .err(),
    ...
    assert!(
        client_exporter
            .derive(&mut client_secret, b"label", None)
            .is_ok()
    );
    assert_ne!(client_secret.to_vec(), server_secret.to_vec());
    assert!(
        server_exporter
            .derive(&mut server_secret, b"label", None)
            .is_ok()
    );
    assert_eq!(client_secret.to_vec(), server_secret.to_vec());
...
fn test_tls12_exporter() {
    ...
    for kt in KeyType::all_for_provider(&provider) {
        ...
        // so outlaw them.
        client_exporter
            .derive(&mut [0], b"label", Some(&[0; 0xffff]))
            .unwrap();
        assert_eq!(
            Error::General("excess context length".into()),
            client_exporter
                .derive(&mut [0], b"label", Some(&[0; 0x10000]))
                .unwrap_err()
        );
    }
}
...
fn test_tls13_exporter_maximum_output_length() {
    ...
    assert_eq!(
        client_exporter
            .derive(&mut too_long_output, b"label", Some(b"context"),)
            .err(),
...
fn early_data_is_available_on_resumption() {
    ...
    let client_secret = client_early_exporter
        .derive([0u8; 32], b"label", Some(b"context"))
        .unwrap();
